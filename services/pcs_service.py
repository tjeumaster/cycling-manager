...
class PcsService:
    ...
    async def fetch_races_list(self, year: int, circuit: int, cls: str) -> list[PcsRace]:
        ...
        async with httpx.AsyncClient(headers=headers, follow_redirects=True, timeout=30.0) as client:
            ...
            for tr in soup.select("table tr"):
                ...
                abs_url = href if href.startswith("http") else f"https://www.procyclingstats.com{href}"
                ...
                if not date:
                    ...
                    m = re.search(r"\b(\d{1,2}\s+[A-Za-zéûôç'-]{3,}\s+\d{4}|\d{4}-\d{2}-\d{2}|\d{1,2}/\d{1,2}/\d{4})\b", row_text, re.I)
                ...
                if date:
                    ...
                    if ddmm:
                        try:
                            d = int(ddmm.group(1))
                            mo = int(ddmm.group(2))
                            date = datetime(year, mo, d, 9, 0, tzinfo=ZoneInfo("Europe/Brussels"))
                        except Exception:
                ...
                if parent_td:
                    ...
                    for _ in range(2):
                        ...
                        cm = re.search(r"\b(1\.UWT|1\.Pro|1\.HC|1\.1|1\.2|2\.1|2\.2|HC|UWT|Pro)\b", stext, re.I)
                        if cm:
                ...
                if not race_class:
                    row_text = tr.get_text(" ", strip=True)
                    cm = re.search(r"\b(1\.UWT|1\.Pro|1\.HC|1\.1|1\.2|2\.1|2\.2|HC|UWT|Pro)\b", row_text, re.I)
                    race_class = cm.group(1) if cm else None
                    
                cat = RaceCategory.WORLD_TOUR if race_class == RaceCircuit.WORLD_TOUR else RaceCategory.OTHER
            ...
            if not races:
                for a in soup.find_all("a", href=True):
                    ...
                    if parent_td:
                        ...
                        m = re.search(r"\b(\d{1,2}\s+(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec|January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{4}|\d{4}-\d{2}-\d{2}|\d{1,2}/\d{1,2}/\d{4})\b", txt, re.I)
                        if m:
                            date = m.group(1)
